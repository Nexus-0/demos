import {Animation, AnimationGroup, ArcRotateCamera, Engine,Mesh,MeshBuilder,Scene, TransformNode, Vector3} from '@babylonjs/core';
...
constructor(private canvas:HTMLCanvasElement){
    ...
    animation(){
        ...
        const cylinderTop = MeshBuilder.CreateCylinder('cylinderTop',{
            height:0.1,
            diameterTop:0,
            diameterBottom:0.12,
            tessellation:4,
        ...
        cylinderBaseB.position = new Vector3(0,.04,0);

        function random(min:number ,max:number) {
            const ratio = 1000;
            return (Math.floor(Math.random() * (max * ratio - min * ratio + 1)) + min * ratio) / ratio;
        ...
        for (let i = 1; i <= 8; ++i){
            ...
            sphere.setPivotPoint(new Vector3(-sphere.position.x,0,-sphere.position.z));
        ...
        const anim01 = new Animation('anim01','position.y',frameRate,Animation.ANIMATIONTYPE_FLOAT,Animation.ANIMATIONLOOPMODE_CYCLE);
        ...
        const anim02 = new Animation('anim02','rotation.y',frameRate,Animation.ANIMATIONTYPE_FLOAT,Animation.ANIMATIONLOOPMODE_CYCLE);
        anim02.setKeys([
            {
                frame:0,
                value:0,
        ...
        const anim03 = new Animation('anim03','rotation.z',frameRate,Animation.ANIMATIONTYPE_FLOAT,Animation.ANIMATIONLOOPMODE_CYCLE);
        anim03.setKeys([
            {
                frame:0,
                value:0,
